...
export const COMMANDS: Command[] = [
  ...
  { name: 'model', description: 'Select a specific model', args: '<provider>/<model>', category: 'model' },
...
export function completeCommand(input: string): string | null {
  ...
  const query = trimmed.slice(1).toLowerCase();
  const matches = COMMANDS.filter(cmd => cmd.name.toLowerCase().startsWith(query));
