...
### Context & Decisions
- Windows builds fail because Rollup cannot `require('@rollup/rollup-win32-x64-msvc')` when lockfiles authored on Linux omit platform-specific optional dependencies (`package.json`, `package-lock.json`, `bun.lock`).
- Bun follows the recorded dependencies exactly, so Windows machines inherit Linux lockfiles that skip Win32 binaries, triggering the runtime error shown in the conversation logs.
- Root-cause aligns with npm optional-dependency bug (npm/cli#4828); mitigation is to explicitly list the necessary Windows Rollup binaries as `optionalDependencies` so every lockfile includes them regardless of author platform.
- Scope limited to 64-bit Windows targets: **skip 32-bit packages** per latest direction; cover `@rollup/rollup-win32-x64-msvc` and `@rollup/rollup-win32-arm64-msvc` only.

### Technical Specifications
| Area | Details |
| --- | --- |
| Build tool wrapper | `script/vite-build.ts` spawns `vite build` and streams filtered logs (no change required). |
| Package metadata | Updates confined to root `package.json` `optionalDependencies` block; add Rollup Win64 binaries and retain Lightning CSS entries. |
...
### Validation Criteria
- `package.json` shows both Rollup Win64 packages under `optionalDependencies` with version `4.52.5` and no 32-bit entries.
- `bun.lock` and `package-lock.json` each contain `@rollup/rollup-win32-x64-msvc` and `@rollup/rollup-win32-arm64-msvc` entries.
- `bun run build` succeeds on Linux/macOS (baseline) **and** Windows machines after a clean reinstall.
- Windows build logs no longer throw `Cannot find module '@rollup/rollup-win32-x64-msvc'`.
- Documentation includes user-facing remediation steps, reducing future support churn.
